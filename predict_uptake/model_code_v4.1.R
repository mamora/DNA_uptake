...
prob.model_v4.1_p1<- function(a, b = b, w = w, sd = sd, b0 = b0, u0 = u0 ){
    ...
    for(i in one.or.more){    # calculate Buss and Uuss for each fragment alignment                                              #                                                                                                      #
      ...
      if(length(temp) == 1){  # for fragment alignments with 1 USS                                                               #        
        ...
        if(length(temp) >= 2){ 
          ...
          # if there is more than 1 uss in a fragment alignment                                            #
          ...
          if (length(temp) >= 2) { 
            dif.pb<- (1 - p.bind[i])
            adjust<- ((USS_separation/sd)*dif.pb)/max(USS_separation/sd)            #
            p.bind[i]<- p.bind[i] + adjust[USS_separation %in% separ]                                                      #  
            #
                                                             #
                                                                                                                                #                                                                                                                    #  
          }                                                                                                                      #
  ...
  if(length(temp) == 0) {
      ...
      if(sd >= w){                                                                                                               #
        ...
        if (length(temp) >= 2) {                                                                                               #
          dif.pb<- (1 - p.b)
          adjust<- ((USS_separation/sd)*dif.pb)/max(USS_separation/sd)            #
          p.b<- p.b + adjust[USS_separation %in% separ]
...
prob.model_f_v4.1 <- compiler::cmpfun(loop_the_genome)
