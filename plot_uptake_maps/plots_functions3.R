...
standard.plot <- function(data = simple_model, theme = theme_background_grey,  add.arrows = TRUE){
  p<- data %>% dplyr::filter(pos >= start.pos & pos <= end.pos) %>% 
    tidyr::gather("samples", "uptake", 2:3)   
  p$samples<- factor(p$samples, levels= c("observed_uptake","expected_uptake"))
    p$samples <- as.character(p $samples) 
    
    p0<- p %>% ggplot() +
    geom_line(aes(x = pos, y = uptake, colour = samples ), size = 1) +
    geom_line(data = subset(p, samples == 'observed_uptake'),
              aes(x = pos, y = uptake, colour = rev(samples)), size = 1) +
...
#############################################################################################
# Function to plot uptake map of observed vs predicted uptake in scientific notation xaxis  #
#############################################################################################
...
##########################################################################################
# Function to plot uptake map of observed vs predicted uptake using log2 of the y axis  #
##########################################################################################
...
log2.plot <- function(data = simple_model, theme = theme_background_grey,  add.arrows = TRUE){ 
  ...
  log.p<- p %>% ggplot() +
    geom_line(aes(x = pos, y = log2(uptake), colour = samples ), size = 1) +
    geom_line(data = subset(p, samples == 'observed_uptake'),
              aes(x = pos, y = log2(uptake), colour = rev(samples)), size = 1) +
    scale_x_continuous(breaks = x.axis.num, expand = c(0, 0), 
