...
for (i in 1:length(u)){
  p<- Uptake.ratio.np.all %>% dplyr::filter(sample == u[i]) %>% 
    ...
    theme(plot.margin=unit(c(1,1,1,1),"cm"),
          ...
          axis.text  = element_text(size=18),
          axis.title = element_text(size = 18, face = "bold")) 
  file_name = paste("C:/Users/marcelo/Dropbox/uptake/Uptake_summer2017/Figures/replicates_uptake/uptake_maps","whole_genome", u[i], "tiff", sep=".")
  tiff(file_name, width = 1000, height = 700, units = "px")
  print(p)
  dev.off()  
}
...
for (i in 1:length(u)){
  p<- Uptake.ratio.gg.all %>% dplyr::filter(sample == u[i]) %>% 
    ...
    ggtitle(" coverage maps PittHH donor DNA") +
    theme(plot.margin=unit(c(1,1,1,1),"cm"),
          legend.position = "bottom",
          panel.grid.minor = element_line(colour="white", size=0.5),
          plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
